...
class Exchange_Rates_ECB_Historical_Model extends Exchange_Rates_ECB_Model {
	...
	private $ecb_api_rates_last_90_days_url = 'http://www.ecb.europa.eu/stats/eurofxref/eurofxref-hist-90d.xml';
	...
	protected function decode_rates($ecb_rates) {
		...
		foreach($ecb_rates->Cube->Cube as $rates_for_day) {
			if($rates_for_day['time'] != $this->target_date) {
				continue;
			}

			// Debug
			//var_dump($rates_for_day);die();
			foreach($rates_for_day->Cube as $rate) {
