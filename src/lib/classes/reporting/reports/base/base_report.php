...
class Base_Report extends \WC_Admin_Report {
	...
	protected function create_temporary_table($table_name, $sql) {
		...
		try {
			...
			if($result === false) {
				$message = sprintf(__('Creation of temporary table "%s" failed. Please ' .
															'check PHP error log for error messages ' .
															'related to the operation.', $this->text_domain),
													 $table_name);
				$this->log($message, false);
				trigger_error(E_USER_WARNING, $message);
			}
			else {
				$this->log(sprintf(__('Table "%s" created successfullly.', $this->text_domain),
													 $table_name));
				$result = true;
			}
		}
		catch(Exception $e) {
			$message = sprintf(__('Unexpected error occurred while creating temporary ' .
														'table "%s". Error message: "%s". Please' .
														'check PHP error log for further details ' .
														'related to the operation.', $this->text_domain),
												 $table_name,
												 $e->getMessage());
			$this->log($message, false);
	...
	public function __construct() {
		...
		$this->debug = WC_Aelia_EU_VAT_Assistant::instance()->debug_mode();
